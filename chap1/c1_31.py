def recursive_process(operator,init,term,a,follow,b):
    if a>b:
        ...
        return operator(term(a),recursive_process(operator,init,term,follow(a),follow,b))

def iterative_process(operator,state,term,a,follow,b):
    if a>b:
        return state
    else:
        return iterative_process(operator,operator(state,term(a)),term,follow(a),follow,b)
